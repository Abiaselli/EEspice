...
constexpr int m_size = 1000 * 8;  // Must be a multiple of 8.
 ...
 */
int VerifyResult(float (*c_back)[P], buffer<float, 2> &a, buffer<float, 2> &b);
...
int main() {
  ...
  try {
    ...
    cout << "Time taken by function: " << duration.count() << " ns \n";
  ...
  result = VerifyResult(c_back, a_buf, b_buf);
  delete[] c_back;
...
int VerifyResult(float (*c_back)[P], buffer<float, 2> &a, buffer<float, 2> &b) {
  ...
  host_accessor a_acc(a);
  host_accessor b_acc(b);

  // Each element of matrix a is 1.
  for (i = 0; i < M; i++)
    for (j = 0; j < N; j++) a_host[i][j] = a_acc[i][j];

  // Each column of b_host is the sequence 1,2,...,N
  for (i = 0; i < N; i++)
    for (j = 0; j < P; j++) b_host[i][j] = b_acc[i][j];
