...
bool isDebugMode();
void R_assigner(int node_x, int node_y, double G, arma::mat &LHS, arma::mat &RHS);
...
std::pair<arma::mat,arma::mat> DynamicNonLinear(const CKTcircuit &ckt, double h, arma::mat pre_solution, int mode, const double time_trans, std::vector<Capacitor> &C_list, int NR_iteration_counter);
...
public:
    ...
    arma::mat LTE_bound_upup{};
    arma::mat LTE_bound_downdown{};

    arma::mat LTE_BE_mid_h{};
    arma::mat LTE_BE_up_h{};
    arma::mat LTE_BE_down_h{};
    arma::mat LTE_BE_upup_h{};
    arma::mat LTE_BE_downdown_h{};
};
...
public:
    ...
    arma::mat C_current_upup;
    arma::mat C_voltage_upup;
    arma::mat C_charge_upup;
    arma::mat Capacitance_upup;

    arma::mat C_current_downdown;
    arma::mat C_voltage_downdown;
    arma::mat C_charge_downdown;
    arma::mat Capacitance_downdown;
    ...
    arma::mat RHS_down; 
    arma::mat RHS_upup;
    arma::mat RHS_downdown;

    arma::mat LHS_mid;
    arma::mat LHS_up;
    arma::mat LHS_down;
    arma::mat LHS_upup;
    arma::mat LHS_downdown;

    arma::mat solution_mid;
    arma::mat solution_up;
    arma::mat solution_down;
    arma::mat solution_upup;
    arma::mat solution_downdown;
    ...
    std::vector<Capacitor> C_list_upup;
    std::vector<Capacitor> C_list_downdown;
...
void R_assigner(int node_x, int node_y, double G, arma::mat &LHS, arma::mat &RHS){
    ...
    if((node_x == 0) && (node_y == 0)){
        ...
        if(node_x == 0){
            LHS.row(node_y-1).col(node_y-1) += x;
        }
        else if(node_y == 0){
            LHS.row(node_x-1).col(node_x-1) += x;
        }
        else{
            LHS.row(node_x-1).col(node_x-1) += x;
            LHS.row(node_x-1).col(node_y-1) += -x;
            LHS.row(node_y-1).col(node_x-1) += -x;
            LHS.row(node_y-1).col(node_y-1) += x;
        }
    }
                      ...
                      arma::mat solution, int mode)
{
    ...
    Is_assigner(node_x, node_y, I_eq, LHS, RHS);
    // Matrix stamp for a diode on LHS
    R_assigner(node_x, node_y, G_eq, LHS, RHS);
...
double NMOS_assigner(int number, int node_vd, int node_vg, int node_vs, int node_vb, double W, double L, double h,
                    const arma::mat solution , int T_nodes, arma::mat &LHS, arma::mat &RHS, int mode, std::vector<Capacitor> &C_list, int NR_iteration_counter)
{
    ...
    // # the settings for fet model based on the large signal analysis
        ...
        R_assigner(node_vd, T_nodes - (3 * number) + 1, 1.0/RD, LHS, RHS); // # RD
        R_assigner(node_vg, T_nodes - (3 * number) + 2, 1.0/RG, LHS, RHS);   // # RG
        R_assigner(T_nodes - (3 * number) + 3, node_vs, 1.0/RS, LHS, RHS); // # RS
    ...
    I_DSeq = id - gds * vds - gm * vgs; // # 10.190 equation
        ...
        Is_assigner(T_nodes - (3 * number) + 1, T_nodes - (3 * number) + 3, I_DSeq, LHS, RHS);
        R_assigner(T_nodes - (3 * number) + 1, T_nodes - (3 * number) + 3, gds, LHS, RHS);           // # assigning gds
                    ...
                    const arma::mat solution , int T_nodes, arma::mat &LHS, arma::mat &RHS, int mode, std::vector<Capacitor> &C_list, int NR_iteration_counter)
{
    ...
    // # the settings for fet model based on the large signal analysis
        ...
        R_assigner(T_nodes - (3 * number) + 1, node_vd, 1.0/RD, LHS, RHS); // # RD
        R_assigner(node_vg, T_nodes - (3 * number) + 2, 1.0/RG, LHS, RHS);   // # RG
        R_assigner(node_vs, T_nodes - (3 * number) + 3, 1.0/RS, LHS, RHS); // # RS
    ...
    I_DSeq = id - gds * vsd - gm * vsg;
        ...
        Is_assigner(T_nodes - (3 * number) + 3, T_nodes - (3 * number) + 1, I_DSeq, LHS, RHS);
        // VCCS_assigner(node_vs, node_vd, node_vs, node_vb, gmb, LHS); // assigning gmb
        R_assigner(T_nodes - (3 * number) + 3, T_nodes - (3 * number) + 1, gds, LHS, RHS);           // # assigning gds
...
std::pair<arma::mat,arma::mat> DynamicNonLinear(const CKTcircuit &ckt, double h, arma::mat pre_solution, int mode,const double time_trans, std::vector<Capacitor> &C_list, int NR_iteration_counter){
    ...
    const arma::mat &J_x = LHS;
    const arma::mat &F_x = RHS;
    // J_x.print("J_x matrix =");
    // F_x.print("F_x matrix =");
    ARMA_PRINT(J_x,"J_x matrix =");
    ARMA_PRINT(F_x,"F_x matrix =");
...
arma::mat NewtonRaphson_system(const CKTcircuit &ckt, const arma::mat &pre_solution, const double &h, const int &mode, const double time_trans, std::vector<Capacitor> &C_list){
    ...
    do{
        ...
        // std::cout << "rcond_LHS is: " << rcond_LHS << std::endl;
        // DEBUG_PRINT("rcond_LHS is: " << rcond_LHS);
        // if(rcond_LHS < 1e-15){
        //     std::cout << "rcond_LHS is smaller than 1e-15 in NR" << std::endl;
        //     exit(1);
        // }
        // Solve Ax = b
        ...
        // solution.print("Solution in NR =");
        // ARMA_PRINT(solution, "Solution in NR =");
    ...
    // auto endNR = std::chrono::high_resolution_clock::now();
    // totalNR += (endNR - startNR);
...
void timestep_options(double & temp_h, double & next_h_up, double & next_h_down, double &next_h_upup, double &next_h_downdown,
    const Transient &trans, bool & TMAX_reach, bool & TMIN_reach) {
    ...
    next_h_up = temp_h * 2.0;
    next_h_down = temp_h / 2.0;

    next_h_upup = temp_h *1.5;
    next_h_downdown = temp_h / 4.0;
    ...
    if(next_h_upup > trans.h_MAX){
        next_h_upup = trans.h_MAX;
    }
    else if(next_h_upup < trans.h_MIN){
        next_h_upup = trans.h_MIN;
    }
    if(next_h_downdown > trans.h_MAX){
        next_h_downdown = trans.h_MAX;
    }
    else if(next_h_downdown < trans.h_MIN){
        next_h_downdown = trans.h_MIN;
    }
...
int h_reach_new(const Truncation_error LTE, int &last_decision, const multi_timestep &multi_h){
    ...
    int reach{};
        ...
        bool upup = multi_h.h_upup <= TRTOL * LTE.LTE_BE_upup_h.min();
        bool downdown = multi_h.h_downdown <= TRTOL * LTE.LTE_BE_downdown_h.min();
        ...
        else if(mid == true && up == true && down == true && upup == true && downdown == true){

            reach = 3;  // up
            return reach;
        }

        else if(mid == true && up == false && down == true && upup == true && downdown == true){
            reach = 4;  // upup
            return reach;
        }

        else if(mid == true && up == false && down == true && upup == false && downdown == true){
            reach = 2;  // mid
            return reach;
        }

        else if(mid == false && up == false && down == true && upup == false && downdown == true){
            last_decision = 0;
            reach = 1;  // down
            return reach;
        }

        else if(mid == false && up == false && down == false && upup == false && downdown == true){
            reach = 5;  // downdown
            return reach;
        }
        else
...
multi_timestep multi_solution_solver(const double &h, const Transient &trans, const CKTcircuit &ckt){
    ...
    timestep_options(multi_h.h_mid, multi_h.h_up, multi_h.h_down, multi_h.h_upup, multi_h.h_downdown, trans, multi_h.TMAX_reach, multi_h.TMIN_reach);  // calculate the next time step, up mid down
    ...
    multi_h.t_upup = trans.time_trans + multi_h.h_upup;
    multi_h.t_downdown = trans.time_trans + multi_h.h_downdown;
    ...
    // }
        ...
        pool.detach_task(
            ...
            {   
                ...
                std::vector<Capacitor> C_list_upup = trans.C_list;
                arma::mat upup_solution = NewtonRaphson_system(ckt, vec_trans.back().solution, multi_h.h_upup, 1, multi_h.t_upup, C_list_upup);

                multi_h.solution_upup = upup_solution;
                std::pair<arma::mat, arma::mat> upup_currents_voltages = get_currents_voltages(C_list_upup, multi_h.h_upup, upup_solution, vec_trans.back().solution);
                multi_h.C_current_upup = upup_currents_voltages.first;
                multi_h.C_voltage_upup = upup_currents_voltages.second;
                arma::mat upup_Capacitance = get_capacitance(C_list_upup);       // Update the capacitance values because the MOSFETs
                multi_h.C_charge_upup = upup_Capacitance % multi_h.C_voltage_upup;  // element-wise multiplication of two objects (Schur product)
                multi_h.Capacitance_upup = upup_Capacitance;
                multi_h.C_list_upup = C_list_upup;
        ...
        pool.detach_task(
            [&trans, &ckt, &multi_h]
            {   
                std::vector<Capacitor> C_list_downdown = trans.C_list;
                arma::mat downdown_solution = NewtonRaphson_system(ckt, vec_trans.back().solution, multi_h.h_downdown, 1, multi_h.t_downdown, C_list_downdown);

                multi_h.solution_downdown = downdown_solution;
                std::pair<arma::mat, arma::mat> downdown_currents_voltages = get_currents_voltages(C_list_downdown, multi_h.h_downdown, downdown_solution, vec_trans.back().solution);
                multi_h.C_current_downdown = downdown_currents_voltages.first;
                multi_h.C_voltage_downdown = downdown_currents_voltages.second;
                arma::mat downdown_Capacitance = get_capacitance(C_list_downdown);       // Update the capacitance values because the MOSFETs
                multi_h.C_charge_downdown = downdown_Capacitance % multi_h.C_voltage_downdown;  // element-wise multiplication of two objects (Schur product)
                multi_h.Capacitance_downdown = downdown_Capacitance;
                multi_h.C_list_downdown = C_list_downdown;
...
arma::mat multi_next_h(Transient &trans, const CKTcircuit &ckt){
    ...
    do{
        ...
        timer.total();
            ...
            arma::mat upup_max_current = arma::max(arma::abs(multi_h.C_current_upup), arma::abs(pre_current));
            arma::mat downdown_max_current = arma::max(arma::abs(multi_h.C_current_downdown), arma::abs(pre_current));
            ...
            arma::mat upup_max_charge = arma::max(arma::abs(multi_h.C_charge_upup), arma::abs(pre_charge));
            arma::mat downdown_max_charge = arma::max(arma::abs(multi_h.C_charge_downdown), arma::abs(pre_charge));
            ...
            arma::mat upup_LTE_current = ABSTOL + RELTOL * upup_max_current;
            arma::mat downdown_LTE_current = ABSTOL + RELTOL * downdown_max_current;
            ...
            arma::mat upup_LTE_charge = RELTOL * arma::max(upup_max_charge, CHGTOL_MAT) / multi_h.h_upup;
            arma::mat downdown_LTE_charge = RELTOL * arma::max(downdown_max_charge, CHGTOL_MAT) / multi_h.h_downdown;
            ...
            LTE.LTE_bound_upup = arma::max(upup_LTE_current, upup_LTE_charge);
            LTE.LTE_bound_downdown = arma::max(downdown_LTE_current, downdown_LTE_charge);
            ...
            arma::mat BEcur_diff_upup = multi_h.C_current_upup - pre_current;
            arma::mat BEcur_diff_downdown = multi_h.C_current_downdown - pre_current;
            ...
            LTE.LTE_BE_upup_h = (2 * multi_h.Capacitance_upup) / arma::abs(BEcur_diff_upup) % LTE.LTE_bound_upup;
            LTE.LTE_BE_downdown_h = (2 * multi_h.Capacitance_downdown) / arma::abs(BEcur_diff_downdown) % LTE.LTE_bound_downdown;
       ...
       switch(index_h){
        ...
        case 3:
            ...
            break;
        case 4:
            trans.h = multi_h.h_upup;
            trans.LHS = multi_h.LHS_upup;
            trans.RHS = multi_h.RHS_upup;
            trans.solution = multi_h.solution_upup;
            trans.C_list = multi_h.C_list_upup;
            trans.Capacitance = multi_h.Capacitance_upup;
            solution = multi_h.solution_upup;

            trans.C_current = multi_h.C_current_upup;
            trans.C_voltage = multi_h.C_voltage_upup;
            trans.C_charge = multi_h.C_charge_upup;
            break;
        case 5:
            trans.h = multi_h.h_downdown;
            trans.LHS = multi_h.LHS_downdown;
            trans.RHS = multi_h.RHS_downdown;
            trans.solution = multi_h.solution_downdown;
            trans.C_list = multi_h.C_list_downdown;
            trans.Capacitance = multi_h.Capacitance_downdown;
            solution = multi_h.solution_downdown;

            trans.C_current = multi_h.C_current_downdown;
            trans.C_voltage = multi_h.C_voltage_downdown;
            trans.C_charge = multi_h.C_charge_downdown;
            break;
