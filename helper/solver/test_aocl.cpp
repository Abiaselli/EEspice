...
void solveAOCL(arma::mat &LHS, arma::vec &RHS){
    ...
    // Check for errors
    if (info < 0) {
        throw std::runtime_error("LAPACKE_dgesv (AOCL): Invalid parameter at position " +
                                 std::to_string(-info));
    } else if (info > 0) {
        throw std::runtime_error("LAPACKE_dgesv (AOCL): Matrix is singular, U(" +
...
int main() {
    ...
    for (int i = 0; i < 5; i++){
        ...
        solveAOCL(LHS_copy, RHS_copy);
    }

    double aocl_time = 0.0;
    double aocl_time_per_solve = 0.0;

    for (int i = 0; i < iter; i++) {
        arma::mat LHS_copy = LHS;
        arma::vec RHS_copy = RHS;

        auto start_aocl = std::chrono::high_resolution_clock::now();
        solveAOCL(LHS_copy, RHS_copy);
        auto end_aocl = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_aocl = end_aocl - start_aocl;
        aocl_time += elapsed_aocl.count();
    }
    aocl_time_per_solve = aocl_time / iter;

    std::cout << "AOCL solver time for size " << size << "x" << size << " over " << iter << " iterations: "
              << aocl_time << " seconds (" << aocl_time_per_solve * 1e6 << " μs per solve)" << std::endl;
