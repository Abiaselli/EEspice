...
int NF;
    ...
    if ((NF%2) != 0)
    {   *nuEndD = *nuEndS = 1.0;
        *nuIntD = *nuIntS = 2.0 * std::max((nf - 1.0) / 2.0, 0.0);
    }
    else
    {   if (minSD == 1) /* minimize # of source */
        {   *nuEndD = 2.0;
        *nuIntD = 2.0 * std::max((nf / 2.0 - 1.0), 0.0);
        *nuEndS = 0.0;
        *nuIntS = nf;
        }
        else
        {   *nuEndD = 0.0;
                *nuIntD = nf;
                *nuEndS = 2.0;
                *nuIntS = 2.0 * std::max((nf / 2.0 - 1.0), 0.0);
        }
    }
return 0;
}
...
BSIM4RdseffGeo(double nf, int geo, int rgeo, int minSD, 
    double Weffcj, double Rsh, double DMCG, double DMCI, double DMDG, int Type, double *Rtot)
{
